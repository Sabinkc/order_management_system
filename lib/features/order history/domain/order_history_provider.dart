...
List<Map<String, dynamic>> get filteredOrders {
  ...
  if (_selectedFilter != 'all_status') {
    switch (_selectedFilter) {
      case "pending":
        statusFilteredOrders = _invoiceOrders.where((order) => order["status"] == "Pending").toList();
        break;
      case "paid":
        statusFilteredOrders = _invoiceOrders.where((order) => order["status"] == "Paid").toList();
        break;
      case "refunded":
        statusFilteredOrders = _invoiceOrders.where((order) => order["status"] == "Refunded").toList();
        break;
      case "cancelled":
        statusFilteredOrders = _invoiceOrders.where((order) => order["status"] == "Cancelled").toList();
        break;
      default:
